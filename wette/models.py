...
PRIZE_DISTRIBUTION = collections.defaultdict(lambda: 0.0, {0: 0.5, 1: 0.3, 2: 0.2})

ScoreboardEntry = collections.namedtuple(
    "ScoreboardEntry", ["points", "rank", "reward"]
)
...
class Challenge(enum.Enum):
    ...
    def calculate_scoreboard(self, users):
        ...
        sorted_users = sorted(
            users, key=lambda user: user.points_for_challenge(self), reverse=True
        )
        ...
        final_reward_for_rank = self.compute_final_reward(
            len(users), ranking, preliminary_rewards
        )

        return {
            user.id: ScoreboardEntry(
                points=points, rank=rank, reward=final_reward_for_rank[rank]
            )
...
class Match(db.Model):
    ...
    first_goal = sa.Column(
        sa.Enum(Outcome, values_callable=_get_values), nullable=False, default="X"
    )
    ...
    def compute_odds(self, num_players):
        ...
        for o in counter.keys():
            counter[o] = (
                num_players / counter[o]
            )  # num_players is always greater than counter
    ...
    def status(self):
        ...
        if self.date > datetime.datetime.now(datetime.timezone.utc).replace(
            tzinfo=None
        ):
...
class User(db.Model):
    ...
    def compute_points(self):
        ...
        challenge_points = {
            challenge: sum(bet_points[challenge] for bet_points in bets_points)
            + champion_points
    ...
    def create_missing_bets(self):
        ...
        matches_without_bets = [
            match for match in all_matches if match not in matches_of_existing_bets
        ]
    ...
    def visible_bets(self):
        ...
        visible_bets = [
            bet for bet in self.bets if bet.valid and not bet.match.editable
        ]
