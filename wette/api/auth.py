...
def register():
    ...
    salted_password = bytes(
        app.config["PASSWORD_SALT"] + posted_login["password"], "utf-8"
    )
    ...
    common.send_mail(
        subject="Neuer Schoselwetter", body=body, recipients=app.config["ADMIN_MAILS"]
    )

    logger.info(
        "User registered: %s, %s, %s", user.first_name, user.last_name, user.email
    )
...
def login():
    ...
    user = db.session.execute(
        sa.select(models.User).filter_by(
            email=posted_login["email"], password=password_hash
        )
    ).scalar()
