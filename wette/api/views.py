...
def users_api():
    ...
    scoreboards = {
        challenge: challenge.calculate_scoreboard(users)
    ...
    return flask.jsonify(
        sorted(
            user_entries,
            key=lambda user_entry: (user_entry["reward"], user_entry["name"]),
        )
    )
...
def matches_api():
    ...
    scheduled_matches_entries = [
        apify_match(match)
        for match in matches
        if match.status == models.Status.SCHEDULED
    ]

    d = {
        "live": live_matches_entries,
...
def status_api():
    ...
    teams = (
        db.session.execute(sa.select(models.Team).order_by(models.Team.name))
        .scalars()
        .all()
    )
...
def apify_user(
    ...
    d = {
        "admin": user.admin,
        "avatar": "https://schosel.net/api/avatar/"
        + quote(user.name + user.avatar_salt),
        "user_id": user.id,
        "name": user.name,
        "paid": user.paid,
        "reward": sum(
            [scoreboards[challenge][user.id].reward for challenge in models.Challenge]
        )
        if user.paid
        else 0.0,
    }
...
def apify_match(match):
    ...
    if not match.editable:
        d["odds"] = {
            models.Outcome.TEAM1_WIN.value: match.odds[models.Outcome.TEAM1_WIN],
...
def apify_challenge(challenge):
    return {"challenge_id": challenge.value, "name": challenge.name}
